...
export function PerformanceChart({ data }: { data: Array<{ label: string; views: number; posts: number }> }) {
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-zinc-500">Visualizações acumuladas dos posts, agrupadas pelo dia em que foram publicados.</p>
        </div>
      ...
      {visible.length === 0 ? (
        ...
        <div className="flex h-40 items-end gap-1.5 overflow-x-auto pb-1">
          {visible.map((item) => (
            <div key={item.label} className="group flex min-w-7 flex-1 flex-col items-center justify-end gap-1">
              ...
              <div className="w-full rounded-t bg-gradient-to-t from-pink-500 via-rose-500 to-orange-400 transition-opacity hover:opacity-80" style={{ height: `${Math.max(4, (item.views / max) * 105)}px` }} />
